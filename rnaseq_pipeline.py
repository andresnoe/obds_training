...
def hisat2(infiles, outfile):
    read1,read2 = infiles
    statement = '''hisat2 -x %(hisat2_index)s
    -1 %(read1)s -2 %(read2)s %(hisat2_options)s
    --threads %(hisat2_threads)s --summary-file %(outfile)s.log
...
@transform(hisat2, regex(r'(.*).bam'),r'\1_.idxstat')
def idxstats(infile, outfile):
    statement = "samtools idxstats %(infile)s > %(outfile)s"
    P.run(statement, job_queue='all.q', job_threads=1, job_memory='2G', job_condaenv="obds-py3")

@transform(hisat2, regex(r'(.*).bam'),r'\1_.flagstat')
