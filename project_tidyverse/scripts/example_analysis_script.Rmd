...
# This will have NAs for crypt numbers that weren't counted for a given mouse

data_processed <- raw_data %>% 
    pivot_longer(-crypt_no, names_to = "mouse_typeofcount", values_to = "count", 
                 values_drop_na = TRUE)
# Ignore crypt_no
# New column = mouse_typeofcount
```
...
```{r}
data_processed <- raw_data %>% 
    # by default, piping a data object into a function places it into the function's first argument
    pivot_longer(-crypt_no, names_to = "mouse_typeofcount", values_to = "count", 
                 values_drop_na = TRUE) %>% 
    separate(mouse_typeofcount, c("Mouse_ID", "count_type"), sep = "_") %>% 
    pivot_wider(names_from = count_type, values_from = count)                                       
